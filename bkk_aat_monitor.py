...
<div style="background:#e0f2f1;border:2px solid #00695c;border-radius:12px;
  ...
  <a href="{best['gf_link']}" target="_blank"
     style="display:inline-block;margin-top:10px;background:#00695c;color:white;
    ...
    for dep in sorted(all_results):
        ...
        for rank, (airline, info) in enumerate(ranked):
            ...
            cells += (f'<td style="padding:6px 10px;vertical-align:top">'
                      f'{medal} <b style="color:#00695c">&#3647;{info["price"]:,}</b> '
...
<h3 style="color:#00695c;margin-top:24px">&#128200; กราฟราคาอันดับ 1 ตามวันออกเดินทาง</h3>
...
def build_chart_png(all_results: dict[date, list]) -> bytes | None:
    ...
    ax.plot(x, ys, marker="o", color="#00695c", linewidth=2, label="ถูกสุดแต่ละวัน")
    ax.set_xticks(x)
