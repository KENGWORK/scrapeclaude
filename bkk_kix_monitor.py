...
def cheapest_direct_per_airline(body: str, gf_link: str) -> dict[str, dict]:
    """Cheapest fare per airline, nonstop (or undetected) only.

    URL pre-filters nonstop; here we drop fares the text marks as >=1 stop.
    Undetected stops are kept (URL trusted).
    """
...
def build_best_box(best: dict | None) -> str:
    ...
    return f"""
<div style="background:#fce4ec;border:2px solid #ad1457;border-radius:12px;
            padding:16px 20px;margin:16px 0">
  <div style="color:#ad1457;font-size:13px;font-weight:bold">&#127942; ช่วงที่ถูกที่สุด (บินตรง)</div>
  <div style="font-size:28px;font-weight:900;color:#ad1457;margin:4px 0">
    &#3647;{best['price']:,}
  ...
  <a href="{best['gf_link']}" target="_blank"
     style="display:inline-block;margin-top:10px;background:#ad1457;color:white;
    ...
    for dep in sorted(all_results):
        ...
        for rank, (airline, info) in enumerate(ranked):
            ...
            cells += (f'<td style="padding:6px 10px;vertical-align:top">'
                      f'{medal} <b style="color:#ad1457">&#3647;{info["price"]:,}</b> '
...
<h2 style="color:#ad1457">&#9992;&#65039; ราคาตั๋ว BKK -> KIX (ไป-กลับ 7 วัน / 6 คืน, บินตรง)</h2>
<p style="color:#555">ข้อมูล ณ {now} | ราคา THB ต่อคน รวมภาษี | แสดง 3 สายการบินถูกสุดต่อวัน (เฉพาะบินตรง)</p>
{best_box}
<h3 style="color:#ad1457;margin-top:20px">&#128203; ราคาทุกช่วง</h3>
...
<h3 style="color:#ad1457;margin-top:24px">&#128200; กราฟราคาอันดับ 1 ตามวันออกเดินทาง</h3>
...
def build_chart_png(all_results: dict[date, list]) -> bytes | None:
    ...
    ax.plot(x, ys, marker="o", color="#ad1457", linewidth=2, label="ถูกสุดแต่ละวัน")
    ax.set_xticks(x)
    ax.set_xticklabels([d.strftime("%d %b") for d in deps],
                       rotation=45, ha="right", fontsize=8)
    ax.set_ylabel("Price (THB)")
    ax.set_title("BKK -> KIX nonstop round-trip cheapest price by departure date")
    ax.legend()
...
async def main():
    ...
    try:
        subject = f"BKK-KIX ราคาวันนี้ (บินตรง) | {datetime.now().strftime('%d/%m/%Y')}"
